        ...
        <section id="projects" className="py-24 md:py-32 bg-background relative overflow-hidden border-t border-border">
            <div className="container mx-auto px-6 lg:px-8 max-w-[1400px]">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {featured.map((f) => (
                        <div key={f.id} className="p-1.5 bg-black/[0.04] ring-1 ring-black/5 rounded-[1.75rem] h-full">
                            <div className="bg-surface rounded-[1.25rem] overflow-hidden border border-black/5 flex flex-col h-full">
                                <div className="aspect-[4/2.6] relative overflow-hidden bg-[#EDE8E0]">
                                    ...
                                    <div className="absolute top-3 left-3 flex items-center gap-2">
                                        <span className="px-2.5 py-1 rounded-full bg-white/90 backdrop-blur text-[11px] font-mono tracking-wide border border-black/5">{f.kicker}</span>
                                    </div>
                                </div>
                ...
                <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-3">
                    {more.map((m) => (
                        <a key={m.title} href={m.href} target="_blank" rel="noopener noreferrer" className="group flex items-center justify-between gap-4 p-4 rounded-xl border border-border bg-surface hover:bg-black/[0.02] hover:border-black/10 transition-colors">
